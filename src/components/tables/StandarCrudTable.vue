<template>
  <div>
    ...
    >
      ...
      <template v-slot:bottom-row v-if="totalEnable">
        <q-tr style="height: 56px">
          <q-td v-for="spaces in whiteSpaces" :key="spaces"></q-td>
...
<script>
// in this component we can make a simple crud table with some props
// rows: the rows of table
// columns: columns name of your table, remember add an action columns if you want actions
// totalSection: this is for total section , you can set title, stringValues and whiteSpaces
// stringValues: this number is used for delete first strings values of the first row Object
// whiteSpaces: this value is used to create a td in the total section (if you get more strings values in your row object you can add more spaces )
...
export default {
  ...
  setup(props) {
    ...
    const { totalTitle, totalEnable, StringValues, whiteSpaces } =
      totalSection.value;
    const {
      state: { ui },
    } = useStore();
    ...
    const getTotal = (templateRows, StringValues) => {
      // select first element from array
      const keysArr = Object.keys(templateRows[0]);

      // delete elements from array
      keysArr.splice(0, StringValues);
      ...
      keysArr.forEach((keyValue) => {
        ...
        templateRows.forEach((rowItem) => {
          total += rowItem[keyValue];
        });
    ...
    const totalItem = getTotal(templateRows, StringValues);
